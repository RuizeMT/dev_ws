  ...
  const std::string &_paramName, const double _defaultVal) const
{
  ...
  {
    // RCLCPP_INFO_STREAM("Parameter <" << _paramName << "> not found: "
    //                 "Using default value of <" << _defaultVal << ">.");
    return _defaultVal;
  }

  double val = _sdfPtr->Get<double>(_paramName);
  // RCLCPP_DEBUG_STREAM("Parameter found - setting <" << _paramName <<
  //                 "> to <" << val << ">.");
  return val;
}
...
{
  // RCLCPP_DEBUG("Loading usv_gazebo_dynamics_plugin");
      ...
      !_sdf->GetElement("bodyName")->GetValue())
  {
    ...
    // RCLCPP_INFO_STREAM("Did not find SDF parameter bodyName");
  }
  else
  {
    ...
    // RCLCPP_DEBUG_STREAM("Found SDF parameter bodyName as <" << linkName<< ">");
  }
  if (!this->link)
  {
    // RCLCPP_FATAL("usv_gazebo_dynamics_plugin error: bodyName: %s does not exist\n",
    //   linkName.c_str());
    return;
  }
  else
  {
    // RCLCPP_DEBUG_STREAM("USV Dynamics Model Link Name = " << linkName);
  }
  ...
  {
    int defaultVal = 2;
    // RCLCPP_INFO_STREAM("Parameter <length_n> not found: "
  ...
  // RCLCPP_DEBUG("USV Dynamics Parameters: From URDF XACRO model definition");
  // RCLCPP_DEBUG_STREAM("Vessel Mass (rigid-body): " << kMass);
  // RCLCPP_DEBUG_STREAM("Vessel Inertia Vector (rigid-body): X:" << kInertia[0] <<
...
void UsvDynamicsPlugin::Update()
{
  ...
  #endif
  // RCLCPP_DEBUG_STREAM_THROTTLE(0.5, "Vel linear: " << kVelLinearBody);
  ...
  #endif
  // RCLCPP_DEBUG_STREAM_THROTTLE(0.5, "Vel angular: " << kVelAngularBody);
  ...
  // RCLCPP_DEBUG_STREAM_THROTTLE(0.5, "Accel linear: " << kAccelLinearBody);
  const ignition::math::Vector3d kAccelAngularBody =
    (kVelAngularBody - this->prevAngVel) / dt;
  this->prevAngVel = kVelAngularBody;
  // RCLCPP_DEBUG_STREAM_THROTTLE(0.5, "Accel angular: " << kAccelAngularBody);
  ...
  // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "stateDot: \n" << stateDot);
  // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "amassVec :\n" << kAmassVec);
  ...
  // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "Dmat :\n" << Dmat);
  const Eigen::VectorXd kDvec = -1.0 * Dmat * state;
  // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "Dvec :\n" << kDvec);
  ...
  // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "forceSum :\n" << kForceSum);
  ...
  // For each length segment
    for (int jj = 1; jj <= this->paramLengthN; jj++)
    {
      ...
      // Debug
      // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "[" << ii << "," << jj <<
      //     "] grid points" << bpnt.x() << "," << bpnt.y() << "," << bpnt.z());
      // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "v frame euler " << kEuler);
      // RCLCPP_DEBUG_STREAM_THROTTLE(1.0, "in water frame" << bpntW.x() << "," <<
      //     bpntW.y() << "," << bpntW.z());

      // Vertical location of boat grid point in world frame
      const float kDdz = kPose.Pos().Z() + bpntW.z();
      // RCLCPP_DEBUG_STREAM("Z, pose: " << kPose.Pos().Z() << ", bpnt: "
      //   << bpntW.z() << ", dd: " << kDdz);
      ...
      // RCLCPP_DEBUG_STREAM("buoyForce: " << kBuoyForce);
